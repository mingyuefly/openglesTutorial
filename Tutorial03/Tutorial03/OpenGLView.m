...
-(void)setupProjection
{
    ...
    ksMatrixLoadIdentity(&_projectionMatrix);//设置投影矩阵为单位矩阵
    ksPerspective(&_projectionMatrix, 60.0f, aspect, 1.0f, 10.0f);
...
-(void)updateTransform
{
    ...
    ksMatrixLoadIdentity(&_modelViewMatrix);//设置模型矩阵为单位矩阵
...
-(void)drawTriCone
{
    ...
    GLubyte indices[] = {
        0, 1, 1, 2, 2, 3, 3, 0,
        4, 0, 4, 1, 4, 2, 4, 3,
    };//因为下边选择了GL_LINES直线绘制，所以两点描述一条直线,因此在shader中设置片元为红色，也只填充了线段
...
-(void)render
{
    ...
    glClearColor(0, 1.0, 0, 1.0);//设置绿色背景
