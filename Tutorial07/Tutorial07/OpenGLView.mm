...
-(ISurface *)createSurface:(int)surface;
-(void)updateSurface;
...
-(void)setupBuffers
{
    ...
    // Set color render buffer as a current render buffer
    glBindRenderbuffer(GL_RENDERBUFFER, _colorRenderBuffer);
}
...
-(void)getSlotsFromProgram
{
    ...
    _lightPositionSlot = glGetUniformLocation(_programHandle, "vLightPosition");//光源位置
    _shininessSlot = glGetUniformLocation(_programHandle, "shininess");//镜面光强
    _ambientSlot = glGetUniformLocation(_programHandle, "vAmbientMaterial");//环境光
    _specularSlot = glGetUniformLocation(_programHandle, "vSpecularMaterial");//镜面光
    _emissionSlot = glGetUniformLocation(_programHandle, "vEmissionMaterial");//发射光（来自物体本身）
    _diffuseSlot = glGetAttribLocation(_programHandle, "vDiffuseMaterial");//漫反射光
...
-(void)setupProjection
{
    ...
    // Generate a perspective matrix with a 60 degree
