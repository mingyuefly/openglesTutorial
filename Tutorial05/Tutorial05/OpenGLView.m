...
{
    ksMatrix4 _shouldModelViewMatrix;//胳膊模型视图矩阵
    ksMatrix4 _elbowModelViewMatrix;//手臂模型视图矩阵
...
-(void)drawCube:(ksColor)color
{
    ...
    glVertexAttrib4f(_colorSlot, color.r, color.g, color.b, color.a);//描绘模型时使用改槽位设置模型的颜色。flat着色
...
-(void)drawColorRectangle
{
    ...
    glVertexAttribPointer(_colorSlot, 4, GL_FLOAT, GL_FALSE, 7 * sizeof(float), indices);
    glEnableVertexAttribArray(_positionSlot);
...
-(void)drawColorCube
{
    GLfloat vertices[] = {// 前三个是位置信息，后四个是颜色信息
    ...
    glVertexAttribPointer(_positionSlot, 3, GL_FLOAT, GL_FALSE, 7 * sizeof(float), vertices);//建立CPU和GPU的逻辑连接,从而实现CPU数据上传到GPU。
    glVertexAttribPointer(_colorSlot, 4, GL_FLOAT, GL_FALSE, 7 * sizeof(float), vertices + 3);//顶点信息不仅可以包括坐标信息，还可以包括颜色信息等等,如法线坐标，纹理坐标。这里是smooth着色
    glEnableVertexAttribArray(_positionSlot);//允许顶点着色器读取GPU端（服务端）数据。
    glEnableVertexAttribArray(_colorSlot);
    glDrawElements(GL_TRIANGLES, sizeof(indices)/sizeof(GLubyte), GL_UNSIGNED_BYTE, indices);
    glDisableVertexAttribArray(_colorSlot);
}
...
-(void)render
{
    ...
    glClearColor(0.0, 1.0, 0.0, 1.0);//背景色为绿色
